...
solve. We then use an integration process that will first be solved with the
initial guess we made for the energy eigenvalue (defined in 'generate_data.py'
and saved in 'values_deuteron.jsonl'.
...
def error_E(E_guess):
    ...
    result in the ending point with the desired boundary condition.
...
energy_value = root_scalar(error_E, method='secant', x0=get.energy_guess())
E_binding = energy_value.root
print(E_binding)

# Get the eigenfunction with proper E value
solution_final = solve_ivp(lambda r, y: eq.radial_equation(r, y, E_binding), get.range_of_radius(), get.initial_conditions(), method = 'RK45', max_step = 0.01)
