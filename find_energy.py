"""Here we define the root finding process to achieve the proper eigenvalue.

We take the function 'radial_equation' from schro_equation.py, which contains
the system we want to solve. We then solve it inside the 'error_E' function
through an integration method with the inserted energy value.

So, to carry out the integration we make use of the next function from SciPy:
    - integrate.solve_ivp --> solves the system with the introduced E value
"""
